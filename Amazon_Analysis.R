...
# predict
amazon_nb_pca_predictions <- predict(final_wf_nb,
                                 new_data = amaz.test,
                                 type = "prob") %>%
  mutate(ACTION = ifelse(.pred_1>.5,1,0)) %>%
  bind_cols(., amaz.test) %>% #Bind predictions with test data
...
vroom_write(x=amazon_nb_pca_predictions, file="./AmazonNBPCAPreds.csv", delim=",")
...
# Set up K-fold CV
folds <- vfold_cv(amaz.train, v = 3, repeats = 1)
# Run the CV
CV_results_knn <-knn_wf  %>%
  tune_grid(resamples = folds,
            grid = tuning_grid_knn,
...
# predict
amazon_knn_pca_predictions <- predict(final_wf_knn,
                                  new_data = amaz.test,
                                  type = "prob") %>%
  mutate(ACTION = ifelse(.pred_1>.5,1,0)) %>%
  bind_cols(., amaz.test) %>% #Bind predictions with test data
...
vroom_write(x=amazonlog_predictions, file="./AmazonLogBalPreds.csv", delim=",")
...
vroom_write(x=amazonlogpen_predictions, file="./AmazonLogPenBalPreds.csv", delim=",")
...
vroom_write(x=amazon_rf_predictions, file="./AmazonRFBALPreds.csv", delim=",")
...
vroom_write(x=amazon_nb_predictions, file="./AmazonNBBALPreds.csv", delim=",")
...
vroom_write(x=amazon_knn_predictions, file="./AmazonKNNBALPreds.csv", delim=",")
...
vroom_write(x=amazon_nb_pca_predictions, file="./AmazonNBPCABALPreds.csv", delim=",")
...
vroom_write(x=amazon_knn_pca_predictions, file="./AmazonKNNPCABALPreds.csv", delim=",")
...
vroom_write(x=amazon_svm_lin_predictions, file="./AmazonSVMLINBALPreds.csv", delim=",")
